...
class RoIDataLayer(object):
  ...
  def __init__(self, roidb, num_classes, random=False):
    ...
    print("length of roidb:",len(self._roidb))
  ...
  def _get_next_minibatch_inds(self):
    ...
    if self._cur + cfg.TRAIN.IMS_PER_BATCH >= len(self._roidb):
      self._shuffle_roidb_inds()
    #print("self._perm and length of self._perm:",self._perm,len(self._perm))
    #print("self._cur:",self._cur)
    db_inds = self._perm[self._cur:self._cur + cfg.TRAIN.IMS_PER_BATCH]

    self._cur += cfg.TRAIN.IMS_PER_BATCH
    return db_inds
    # if(len(db_inds)): # added by vikash
    #   return db_inds
    # else:
    #   db_inds = self._perm[0:1] # added by vikash to avoid zero len db_inds
    #   return db_inds
