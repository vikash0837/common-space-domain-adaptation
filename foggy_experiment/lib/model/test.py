...
def test_discriminator(net, imdb, weights_filename, max_per_image=100, thresh=0.):
  ...
  for i in range(num_images):
    ...
    im_blob = blobs['data']
    blobs['im_info'] = np.array([im_blob.shape[1], im_blob.shape[2], im_scales[0]], dtype=np.float32)
    fc7, net_conv,D_img_out = net.test_discriminator(im_blob,blobs['im_info'])
    print("######3 printing result #########")
    print("fc7 shape=:",fc7.size())
    print("net_conv shape=:",net_conv.size())
    print(D_img_out)
    newscore = net.D_img.forward(net_conv)
    print("New score:",newscore)
    print("new score shape=:", newscore.size())
    print("score shape=:",D_img_out.size())
