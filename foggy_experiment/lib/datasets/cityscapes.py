...
class cityscapes(imdb):
  ...
  def image_path_from_index(self, index):
    ...
    #image_path = os.path.join(self._data_path, 'leftImg8bit', self._image_set, 'img',index) # change for custom path
  ...
  def _load_image_set_index(self):
    ...
    #print("###########self._image_set########",self._image_set)
  ...
  def _load_cityscapes_annotation(self, index):
    ...
    """
    #loc = index[:index.find('.')]
    loc = index[:index.find('_')]
    index = index[:index.find('leftImg8bit')]
    #index = index.split('.png')[0] # for fine-tuning
    filename = os.path.join(self._data_path, 'gtFine', self._image_set, loc, index + 'gtFine_polygons.json')
    #filename = os.path.join(self._data_path, 'gtFine', self._image_set,'label', loc + '.json')
    #filename = os.path.join(self._data_path, 'gtFine', self._image_set, loc, index + '.json') # for fientuning
    #print("########filename=:",filename)
    with open(filename, 'r') as f:
        info = json.load(f)
        #info = eval(info) # added by vikash for fine tuning
    objs = info["objects"]
    num_objs = len(objs)
  ...
  def _do_python_eval(self, output_dir='output'):
    ...
    for i, cls in enumerate(self._classes):
      ...
      aps += [ap]
      if(cls not in result_dict.keys()):
        result_dict[cls] = ap
