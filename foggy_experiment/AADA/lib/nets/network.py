...
class Network(nn.Module):
  ...
  def test_discriminator(self, image, im_info):
    ...
    self.eval()
    cls_score, cls_prob, bbox_pred, rois, fc7, net_conv = self.test_image(image, im_info)
    net_conv = grad_reverse(net_conv)
    D_T_score = self.D_img(net_conv)
    self.delete_intermediate_states()
    return cls_score, cls_prob, bbox_pred, D_T_score,net_conv
