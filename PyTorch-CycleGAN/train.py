...
for epoch in range(opt.epoch, opt.n_epochs):
    for i, batch in enumerate(dataloader):
        ...
        pred_fake = netD_B(fake_B)#.squeeze(0).squeeze(0)
        pred_fake = torch.squeeze(pred_fake)
        print(pred_fake.shape, target_real.shape)
        loss_GAN_A2B = criterion_GAN(pred_fake, target_real)

        fake_A = netG_B2A(real_B)
        pred_fake = netD_A(fake_A)#.squeeze(0).squeeze(0)
        pred_fake = torch.squeeze(pred_fake)
        ...
        # Real loss
        pred_real = netD_A(real_A)#.squeeze(0).squeeze(0)
        pred_real = torch.squeeze(pred_real)
        loss_D_real = criterion_GAN(pred_real, target_real)

        # Fake loss
        fake_A = fake_A_buffer.push_and_pop(fake_A)
        pred_fake = netD_A(fake_A.detach())#.squeeze(0).squeeze(0)
        pred_fake = torch.squeeze(pred_fake)
        ...
        # Real loss
        pred_real = netD_B(real_B)#.squeeze(0).squeeze(0)
        pred_real = torch.squeeze(pred_real)
        loss_D_real = criterion_GAN(pred_real, target_real)
        
        # Fake loss
        fake_B = fake_B_buffer.push_and_pop(fake_B)
        pred_fake = netD_B(fake_B.detach())#.squeeze(0).squeeze(0)
        pred_fake = torch.squeeze(pred_fake)
        loss_D_fake = criterion_GAN(pred_fake, target_fake)
    ...
    torch.save(netG_A2B.state_dict(), 'output_F2S/netG_A2B.pth')
    torch.save(netG_B2A.state_dict(), 'output_F2S/netG_B2A.pth')
    torch.save(netD_A.state_dict(), 'output_F2S/netD_A.pth')
    torch.save(netD_B.state_dict(), 'output_F2S/netD_B.pth')
###################################
#return None
#train = torch.nn.DataParallel(train, device_ids=[0, 1])
#output = _net()
#train()
